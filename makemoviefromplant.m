...
writerObj.FrameRate = 1/plants.nominal.robot.Deltat.simpos;
open(writerObj);
for i = 1:mvlgth
  if (mod(i-1,plants.nominal.robot.const.ratiovel)==0)
  ...
  for j = 1:plants.numP
    hold on;
    plants.real(j).robot.x = plants.real(j).Xlog(:,i);
    plot_omunirobotmini(plants.real(j),limit);
    hold on;
  end
  plants.nominal.robot.x = plants.nominal.Xlog(:,i);
